	...
	if Global.single_player == true:
		if (Global.day_and_night % 2) == 0: # ensures the mob only moves during the night.
			...
			if global_position.distance_to(player_pos) > 3:
				...
				if (player_pos.x - global_position.x) < 0: # if the player's x co -ord is less than sleeper mob's:
					$AnimatedSprite2D.play("move_left") # this means player is to the left, so play the corresponding animation.
				else:
		...
		if (Global.day_and_night % 2) == 0: # ensures the mob only moves during the night.
			...
			if global_position.distance_to(player_pos) > global_position.distance_to(player_2_pos) and\
				...
				velocity = player_2_target_position * (speed * (1+(Global.day_and_night)*0.05)) # move toward player 1. (or 2, makes absolutely no difference)
				move_and_slide()
				if (player_pos.x - global_position.x) < 0: # if the player 2's x co -ord is less than sleeper mob's:
					$AnimatedSprite2D.play("move_left") # this means player 2 is to the left, so play the corresponding animation.
				else:
...
func spawn_and_despawn():
	if (Global.day_and_night % 2) != 0 and despawned_tonight == false: # if it is day and this hasn't run before:
		...
		# This error meant that if the user changed to day in the time between the end of the 0.15 seconds and the start of the next calling of first_value, then:
		...
		$Proximity_Area2D/Proximity_CollisionShape2D.set_deferred("disabled", false) # enables the proximity collision shape. 
...
func spawn_in_random_location(): # this spawns the mob in a reandom location, which is no more than 200 pixels away from the player, to keep things fair. otherwise mobs will spawn on the player and instantly kill them, which sucks!
	...
	var distance_to_player = Vector2(random_x, random_y).distance_to(Global.player_position) # finds the distance from the random mob position to the player
	if Global.single_player == false:
		...
		if distance_to_player_2 < 300:
			spawn_in_random_location()
	if distance_to_player < 300: # if the distance to player is less than 300, then: (greater as there is the proximity collisionshape to consider)
		spawn_in_random_location() # try again, this continues until the mob spawns outside of this 150 pixel radius.
		# This prevents the game from being unfair, as if mobs can spawn really close to the player, leaving the player with no time to react, this is unfair. 
	else:
		global_position = Vector2(random_x, random_y) # if the random position is not less than 300 from the player, then spawn there.
